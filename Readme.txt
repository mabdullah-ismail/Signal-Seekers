...
Original Records	29,531 rows x 16 columns
...
Temporal Features
Month indicators with sine/cosine transformations (captures seasonal patterns)

Ratio Features
PM_Ratio = PM2.5 / PM10 — indicates particulate composition

NOx_NO2_Ratio — nitrogen oxide composition

Interaction Features
PM_CO_Interaction = PM2.5 x CO — combined effect of particles and carbon monoxide

NOx_SO2_Interaction = NOx x SO2

Composite Indices
Composite_PM = 0.6xPM2.5 + 0.4xPM10 — weighted particulate matter index

Composite_NOx — weighted nitrogen oxide index

Data Preprocessing
Dropped redundant columns — Removed 'Xylene' (61% missing values)

Handled missing values — Median imputation for all numeric columns

Class balancing — Applied SMOTE (Synthetic Minority Over-sampling Technique) to address class imbalance, resulting in 7,063 samples per class
...
All models showed consistent performance across different random seeds (std dev <= 0.0003)

Random Forest maintained highest accuracy across train-test splits (80-20 to 60-40)
...
text
├── Signal_Seekers_Dileverable_2.ipynb   # Main Jupyter Notebook
├── city_day.csv                         # Dataset file
├── requirements.txt                     # Environment setup instructions
├── Air_Quality_Index_Prediction.pdf     # Project presentation
├── Dileverable2_Signal_Seekers.pdf      # Detailed project report
└── README.md                            # This file
How to Run
Open Google Colab

Upload the notebook Signal_Seekers_Dileverable_2.ipynb

Run the first 2 cells (until # Load Dataset)

Upload city_day.csv when prompted

Run the remaining cells to see results
...
Matplotlib & Seaborn — visualizations (EDA, confusion matrices, ROC curves)
